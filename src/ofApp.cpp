...
//--------------------------------------------------------------
void ofApp::update(){
    ofBackground(ofNoise(255));
   // ofBackground(ofRandom(255),ofRandom(255),ofRandom(255),ofRandom(255));
...
ofVec2f ofApp::calculateV2(vector<boid> Boids, int index){
    ...
    for(int i = 0; i < Boids.size(); i++){
        if(i != index){
            if((Boids[i].pos).distance(Boids[index].pos) < 5){
...
ofVec2f ofApp::calculateV3(vector<boid> Boids, int index){
